...
// Hajj dates must be configured from the current official calendar before release.
...
export const useCurrentGuidance = () => {
  ...
  const updateGuidance = useCallback(() => {
    try {
      ...
      const instruction = guidanceMessage.instruction[language] || guidanceMessage.instruction.en;

...
type InspectorPilgrimInsert = Database['public']['Tables']['inspector_pilgrims']['Insert'];
...
export function useInspectorPilgrims() {
  ...
  const updateGroupWhatsApp = useCallback(async (link: string) => {
    ...
    const { error } = await supabase
      ...
      .eq('id', group.id);
    if (error) {
      toast({ title: 'Could not update WhatsApp link', description: error.message, variant: 'destructive' });
      return;
    }
  ...
  const fetchPilgrims = useCallback(async () => {
    ...
    if (!error) setPilgrims((data || []) as unknown as InspectorPilgrim[]);
  }, [group]);
  ...
  const toggleGroupLeader = useCallback(async (id: string) => {
    ...
    const { error } = await supabase
      ...
      .eq('id', id);
    if (error) {
      toast({ title: 'Could not update group leader', description: error.message, variant: 'destructive' });
      return;
    }
  ...
  const deletePilgrim = useCallback(async (id: string) => {
    const { error } = await supabase.from('inspector_pilgrims').delete().eq('id', id);
    if (error) {
      toast({ title: 'Could not remove pilgrim', description: error.message, variant: 'destructive' });
      return;
    }

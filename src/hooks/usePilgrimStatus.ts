...
export const usePilgrimStatus = (): UsePilgrimStatusResult => {
  ...
  const toggleSharing = useCallback(async (enabled: boolean) => {
    ...
    try {
      ...
      // Withdrawing consent removes the active coordinate immediately. Historic
      // data retention is handled server-side; families must never retain a live pin.
      if (!enabled) {
        const groupId = localStorage.getItem(GROUP_ID_KEY);
        if (groupId) {
      ...
      toast({
        ...
        description: enabled 
          ? "Family can now see your live location while sharing is active"
          : "Live location sharing stopped and your active pin was removed",
      });
    } catch (error) {

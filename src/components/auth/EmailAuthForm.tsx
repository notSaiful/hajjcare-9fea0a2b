...
const getAuthErrorMessage = (error: { message?: string; status?: number; code?: string }) => {
  const code = error.code?.toLowerCase() ?? "";
  const message = error.message?.toLowerCase() ?? "";
  if (code === "auth/email-not-verified" || code === "auth/email-not-confirmed") return "Your email address is not verified. Please check your inbox or spam folder and verify your email.";
  if (code === "auth/invalid-email") return "Invalid email address.";
  if (code === "auth/wrong-password" || code === "auth/invalid-credential" || code === "auth/user-not-found") return "Email or password is incorrect. Please check both, or use Forgot password.";
  ...
  if (code === "auth/email-already-in-use") return "Email already registered. Please sign in instead.";
  if (code === "auth/weak-password") return "Weak password. Use at least 6 characters.";
  if (/already registered|already exists|email.*exist/.test(message)) return "Email already registered. Please sign in instead.";
  if (/email.*not confirmed|email_not_confirmed|confirm your email|not verified/.test(message)) return "Your email address is not verified. Please check your inbox or spam folder and verify your email.";
  if (/invalid login credentials|invalid_credentials|invalid.*password|incorrect.*password/.test(message)) return "Email or password is incorrect. Please check both, or use Forgot password.";
  if (/password/.test(message) && /weak|least|short|length/.test(message)) return "Weak password. Use at least 6 characters.";
  if (/email/.test(message) && /invalid|valid/.test(message)) return "Invalid email address.";
  if (/network|fetch|offline|timeout/.test(message)) return "Network error. Please check your connection and try again.";
  if (error.status && error.status >= 500) return "Server unavailable. Please try again shortly.";
  if (/not configured|configuration/.test(message)) return "Authentication service configuration is invalid. Please contact HajCare support.";
...
export function EmailAuthForm({ onSuccess }: EmailAuthFormProps) {
  ...
  const doSubmit = async () => {
    ...
    try {
      if (isSignUp) {
        const { error, requiresEmailVerification } = await signUp(email, password, fullName, embarkationPoint);
        if (error) {
          console.error("HajCare account creation failed.", error);
          if (error.code === "auth/email-already-in-use" || error.code === "auth/too-many-requests") {
            setVerificationSent(true);
            setIsSignUp(false);
          }
          toast({ title: "Account creation failed", description: getAuthErrorMessage(error), variant: "destructive" });
        ...
        if (error) {
          console.error("HajCare sign-in failed.", error);
          const description = getAuthErrorMessage(error);
          if (error.code === "auth/email-not-verified" || /not verified/i.test(description)) setVerificationSent(true);
          toast({ title: "Sign-in failed", description: isRTL ? "بيانات الدخول غير صحيحة" : description, variant: "destructive" });
        } else {
      ...
      console.error("HajCare authentication request failed unexpectedly.", error);
      const safeError = error instanceof Error
        ? getAuthErrorMessage(error)
        : "Unable to complete authentication. Please try again.";
      toast({ title: "Authentication error", description: safeError, variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      {verificationSent && (
        <div className="space-y-3 rounded-2xl border border-primary/25 bg-primary/5 p-4" role="status" aria-live="polite">
          <div className="flex items-start gap-3">
            <Mail className="mt-0.5 h-5 w-5 shrink-0 text-primary" />
            <div className="space-y-1">
              <p className="text-sm font-semibold text-foreground">Email verification required</p>
              <p className="text-xs leading-5 text-muted-foreground">A verification email has been sent to your email address. Please verify your email before signing in.</p>
            </div>
          </div>
          <Button
            type="button"
            variant="outline"
            className="h-11 w-full rounded-xl border-primary/30 text-primary hover:bg-primary/10"
            disabled={isResendingVerification || !email || !password}
            onClick={async () => {
              setIsResendingVerification(true);
              const result = await resendVerificationEmail(email, password);
              setIsResendingVerification(false);
              if (result.error) {
                toast({ title: "Unable to resend email", description: getAuthErrorMessage(result.error), variant: "destructive" });
              } else if (result.alreadyVerified) {
                setVerificationSent(false);
                toast({ title: "Email verified", description: "Your email is verified. You can sign in now." });
              } else {
                toast({ title: "Verification email sent", description: "Please check your inbox or spam folder." });
              }
            }}
          >
            {isResendingVerification ? <Loader2 className="h-5 w-5 animate-spin" /> : "Resend Verification Email"}
          </Button>
        </div>
      )}
            ...
            <div className="space-y-1.5">
              <div className="relative">
                ...
                <Select value={embarkationPoint} onValueChange={setEmbarkationPoint}>
                  <SelectTrigger aria-label={t.embarkation} className="pl-12 h-14 text-lg rounded-2xl border-border/60 bg-background/60 focus:bg-background transition-colors">

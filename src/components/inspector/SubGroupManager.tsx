...
export const SubGroupManager = () => {
  ...
  const fetchSubGroups = useCallback(async () => {
    ...
    if (error) {
      toast({ title: 'Could not load sub-groups', description: error.message, variant: 'destructive' });
      return;
    }
    setSubGroups(data || []);
  }, [group, toast]);
  ...
  const fetchCheckIns = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .limit(50);
    if (error) {
      toast({ title: 'Could not load check-ins', description: error.message, variant: 'destructive' });
      return;
    }
    setCheckIns(data || []);
  }, [group, subGroups, toast]);
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error } = await supabase.from('inspector_sub_groups').delete().eq('id', id);
    if (error) {
      toast({ title: 'Could not delete sub-group', description: error.message, variant: 'destructive' });
      return;
    }
    await fetchSubGroups();
    await fetchPilgrims();
  ...
  const handleAssignLeader = async (subGroupId: string, pilgrimId: string | null) => {
    const { error: leaderError } = await supabase
      ...
      .eq('id', subGroupId);
    if (leaderError) {
      toast({ title: 'Could not assign leader', description: leaderError.message, variant: 'destructive' });
      return;
    }
    if (pilgrimId) {
      ...
      const { error } = await supabase
        ...
        .eq('id', pilgrimId);
      if (error) {
        toast({ title: 'Could not assign leader', description: error.message, variant: 'destructive' });
        return;
      }
    }
    await fetchSubGroups();
  ...
  const handleAssignMember = async (pilgrimId: string, subGroupId: string | null) => {
    const { error } = await supabase
      ...
      .eq('id', pilgrimId);
    if (error) {
      toast({ title: 'Could not assign member', description: error.message, variant: 'destructive' });
      return;
    }
    await fetchPilgrims();
  };
  ...
  const unassigned = useMemo(
    () => pilgrims.filter(p => !p.sub_group_id),
    [pilgrims]
  );
    ...
    <Card className="border-primary/20">
      ...
      <CardContent className="space-y-3">
        {isLoading ? (
          ...
          subGroups.map((sg) => {
            const members = pilgrims.filter(p => p.sub_group_id === sg.id);
          ...
          {recentCheckIn && (
            <div className="flex items-center gap-1.5 mt-1 text-[11px] text-muted-foreground">
              ...
              <span className={recentCheckIn.status === 'all_present' ? 'text-emerald-600' : 'text-red-600'}>
                {recentCheckIn.status === 'all_present' ? 'all present' : `${recentCheckIn.missing_pilgrim_ids?.length ?? 0} missing`}
              </span>
              {' · '}

...
export function KhidmatTracker({ groupId }: Props) {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('khidmat_logs' as never)
      .select('*')
      .eq('group_id', groupId)
      .gte('log_date', today)
      .order('created_at', { ascending: false });
    if (error) {
      toast({ title: 'Could not load service logs', description: error.message, variant: 'destructive' });
      setLogs([]);
    } else {
      setLogs((data as unknown as KhidmatLog[] | null) ?? []);
    }
    setLoading(false);
  }, [groupId, toast]);

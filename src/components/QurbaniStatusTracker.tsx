...
const QurbaniStatusTracker = () => {
  ...
  const computeStatus = (ref: string): QurbaniStatus => {
    ...
    if (!startedAt) {
      startedAt = Date.now();
      try { localStorage.setItem(`qurbani_started_${trimmedRef}`, String(startedAt)); } catch { /* Storage can be unavailable in private browsing. */ }
    }
  ...
  const runCheck = async (silent = false) => {
    ...
    } catch { /* A status check should still succeed when persistence is unavailable. */ }

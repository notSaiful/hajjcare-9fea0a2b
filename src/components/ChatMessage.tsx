...
const ChatMessage = ({ role, content, isStreaming, languageCode }: ChatMessageProps) => {
  const isUser = role === "user";
  const { language } = useLanguage();
  const responseLanguage = languageCode || language;
      ...
      >
        ...
        {!isUser && !isStreaming && content && (
          <div className="mt-2 flex justify-end">
            <TextToSpeechButton
              text={`${GREETING[responseLanguage] || GREETING.en}${content}`}

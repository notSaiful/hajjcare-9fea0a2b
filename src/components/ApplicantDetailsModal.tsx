...
export const ApplicantDetailsModal = ({
  ...
  useEffect(() => {
    const fetchSignedUrl = async () => {
      ...
      // Legacy rows may contain a public URL. Extract the object path so the
      // now-private bucket is always accessed with a short-lived signed URL.
      const legacyPrefix = "/proof-documents/";
      const objectPath = applicant.proof_url.startsWith("http")
        ? applicant.proof_url.slice(applicant.proof_url.indexOf(legacyPrefix) + legacyPrefix.length)
        : applicant.proof_url;
      if (!objectPath || objectPath === applicant.proof_url && applicant.proof_url.startsWith("http")) {
        setUrlError(true);
        setSignedUrl(null);
        return;
      }

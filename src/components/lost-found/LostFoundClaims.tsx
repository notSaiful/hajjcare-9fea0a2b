...
export const ClaimDialog = ({ open, onOpenChange, report, onSubmitted }: ClaimDialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Could not send claim", description: getErrorMessage(error, "Please try again."), variant: "destructive" });
    } finally {
      setSubmitting(false);
    }
  };
...
export const ClaimsPanel = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const shape = (rows: SafeClaimRow[] | null | undefined): Claim[] =>
      (rows ?? []).map((r) => ({
        ...r,
        report: {
    ...
    setMyClaims(shape(mine as unknown as SafeClaimRow[] | null));
    setIncoming(shape(inc as unknown as SafeClaimRow[] | null));
    setLoading(false);
  }, [user?.id]);

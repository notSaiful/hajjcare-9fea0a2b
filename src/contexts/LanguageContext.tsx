...
// English plus the 22 languages in the Eighth Schedule of the Constitution of India.
// `ar`, `tr` and `ru` remain for existing HajCare content compatibility.
export type Language = "en" | "ar" | "ur" | "hi" | "ta" | "te" | "mr" | "bn" | "or" | "ml" | "kn" | "pa" | "gu" | "as" | "gom" | "ks" | "sa" | "sd" | "doi" | "mai" | "brx" | "sat" | "mni" | "ne" | "tr" | "ru";
...
export const LANGUAGES: { code: Language; name: string; nativeName: string; dir: "ltr" | "rtl" }[] = [
  ...
  { code: "mni", name: "Manipuri (Meitei)", nativeName: "ꯃꯤꯇꯩꯂꯣꯟ", dir: "ltr" },
...
const detectInitialLanguage = (): Language => {
  try {
    const savedLanguage = localStorage.getItem("hajcare-language") as Language | null;
    if (savedLanguage && LANGUAGES.some((item) => item.code === savedLanguage)) return savedLanguage;
  } catch {
    // Continue with the browser preference when storage is unavailable.
  }
  if (typeof navigator !== "undefined") {
    const browserLanguage = navigator.language?.toLowerCase().split("-")[0] as Language | undefined;
    if (browserLanguage && LANGUAGES.some((item) => item.code === browserLanguage)) return browserLanguage;
  }
  return "en";
};
...
export const LanguageProvider = ({ children }: { children: ReactNode }) => {
  ...
  const setLanguage = (nextLanguage: Language) => {
    ...
    try {
      localStorage.setItem("hajcare-language", nextLanguage);
    } catch {
      // Language choice still works for the active session when storage is unavailable.
    }
  };

  // New languages can be released progressively without breaking existing screens.

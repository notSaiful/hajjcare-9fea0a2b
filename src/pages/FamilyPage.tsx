...
const FamilyPage = () => {
  ...
  const { sharingEnabled } = usePilgrimStatus();
  ...
  const activeStage = manualStage || stage;
  const activeStageInfo = HAJJ_STAGES[activeStage];

  // Update location for family group (uses active stage)
  useEffect(() => {
    if (group && sharingEnabled && lat && lng) {
      updateLocation(lat, lng, activeStage);
    }
  }, [group, sharingEnabled, lat, lng, activeStage, updateLocation]);

  // Manual ritual selection is a local accessibility fallback; it never uploads
  // a coordinate unless the pilgrim has separately opted into live sharing.
        ...
        <div className="container max-w-3xl mx-auto px-3 sm:px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sm sm:text-base">
              {isRTL ? "نظام سكون للتتبع" : "Sukoon Tracking System"}
            </span>
          </div>
      ...
      <main className="container max-w-2xl mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
        ...
        {group && (
          <Card className="border-primary/20 bg-primary/5">
            <CardHeader className="pb-2 sm:pb-3 px-4 sm:px-6"><CardTitle className="text-base sm:text-lg">{isRTL ? "خصوصية مشاركة الموقع" : "Live location privacy"}</CardTitle></CardHeader>
            <CardContent className="px-4 sm:px-6"><p className="text-sm text-muted-foreground">{sharingEnabled ? "Live sharing is active. You can pause or stop it any time from Family Status." : "Location sharing is off. Enable it from Family Status before any location is sent to your family."}</p></CardContent>
          </Card>
        )}
        ...
        {/* Sukoon Tracking System – Family Connect: family members' live locations */}
            ...
            <CardHeader className="pb-2 sm:pb-3 px-4 sm:px-6">
              <CardTitle className="flex items-center justify-between text-base sm:text-lg">
                <div className="flex items-center gap-2">
                  ...
                  {isRTL ? "نظام سكون للتتبع – اتصال العائلة" : "Sukoon Tracking System – Family Connect"}
                </div>

...
const getErrorMessage = (error: unknown, fallback: string): string => {
  ...
  if (error && typeof error === "object") {
    const context = (error as { context?: { body?: { error?: unknown } } }).context;
    if (typeof context?.body?.error === "string") return context.body.error;
  }
  return fallback;
};
  ...
  const runOcr = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        const msg =
          getErrorMessage(error, "OCR request failed");
        toast.error(msg);
        return;
      }

      const ocrRows = data && typeof data === "object" && Array.isArray((data as { rows?: unknown }).rows)
        ? ((data as { rows: unknown[] }).rows.filter(isParsedRow))
        : [];
      if (ocrRows.length === 0) {

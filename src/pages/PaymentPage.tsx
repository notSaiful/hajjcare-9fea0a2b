...
export default function PaymentPage() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!window.Razorpay) throw new Error("Payment gateway did not initialize");
      const razorpay = new window.Razorpay(options);
      razorpay.on("payment.failed", async (resp: RazorpayFailureResponse) => {
      ...
      toast({
        title: "Error",
        description: getErrorMessage(error, "Failed to initiate payment"),
        variant: "destructive",
      });
    } finally {
      setIsProcessing(false);

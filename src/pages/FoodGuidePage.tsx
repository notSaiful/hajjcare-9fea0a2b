...
const FoodGuidePage = () => {
  ...
  const content = {
    en: {
      ...
      foodAwarenessSubtitle: "Food guidance for Haj 2027 (1448H) — verify current official advisories before travel",
      rateList: "Rate List",
  ...
  const openGoogleMaps = (lat: number, lng: number, name: string) => {
    window.open(`https://www.google.com/maps/search/?api=1&query=${lat},${lng}&query_place_id=${encodeURIComponent(name)}`, "_blank", "noopener,noreferrer");
  };
          ...
          <Tabs defaultValue="awareness" className="w-full">
            <TabsList className="w-full grid grid-cols-3 sm:grid-cols-6 mb-6 h-auto">
              ...
              <TabsTrigger value="rates" className="text-xs sm:text-sm gap-1 py-2">
                ...
                <span className="hidden sm:inline">{t.rateList || "Rates"}</span>
              </TabsTrigger>
              ...
              <Card className="border-destructive/30 bg-destructive/5">
                <CardHeader>
                  <CardTitle className="text-lg flex items-center gap-2 text-destructive">
                    ...
                    {t.foodAwareness} — Haj 2027 (1448H)
                  </CardTitle>

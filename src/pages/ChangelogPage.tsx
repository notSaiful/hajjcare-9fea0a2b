...
import { fetchRemoteChangelog, type ChangelogItem, type ChangelogItemKind } from "@/lib/changelog";
...
const CHANGELOG_URL = import.meta.env.VITE_CHANGELOG_URL?.trim();
const icons: Record<ChangelogItemKind, typeof Video> = {
  video: Video,
  book: BookOpen,
  security: Shield,
  fix: Wrench,
...
function useRemoteChangelog() {
  return useQuery({
    queryKey: ["remote-changelog", CHANGELOG_URL],
    queryFn: ({ signal }) => fetchRemoteChangelog(CHANGELOG_URL!, signal),
    enabled: Boolean(CHANGELOG_URL),
    staleTime: 5 * 60 * 1000,
...
const ChangelogPage = () => {
  ...
  const fallbackItems = entries[(language in entries ? language : "en") as keyof typeof entries];
  const { data: remoteChangelog } = useRemoteChangelog();
  const releases = remoteChangelog?.releases.map((release) => {
    const translation = release.translations[language] ?? release.translations.en;
    return { id: release.id, heading: translation.heading, date: translation.date ?? release.date, items: translation.items };
  }) ?? [{
    id: "bundled-2026-07-06",
    heading: t.heading,
    date: t.date,
    items: fallbackItems.map((entry) => ({
      kind: entry.icon === Video ? "video" : entry.icon === BookOpen ? "book" : entry.icon === Shield ? "security" : "fix",
      badge: entry.badge,
      title: entry.title,
      body: entry.body,
    })) as ChangelogItem[],
        ...
        <main className="container max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          <div className="space-y-5">
            {releases.map((release) => (
              <Card key={release.id} className="border border-border/60 shadow-card">
                <CardHeader className="pb-4">
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <CardTitle className="text-lg sm:text-xl font-semibold text-foreground">{release.heading}</CardTitle>
                ...
                <CardContent className="space-y-5">
                  {release.items.map((entry, idx) => {
                    const Icon = icons[entry.kind];
                    return (
                      <div key={`${release.id}-${idx}`} className="flex items-start gap-3 sm:gap-4 p-3.5 sm:p-4 rounded-xl bg-muted/30 border border-border/40">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap mb-1">
                            <span className="font-semibold text-sm sm:text-base text-foreground">{entry.title}</span>
                            <Badge variant="secondary" className="text-[10px] sm:text-xs h-5 px-2 rounded-full">{entry.badge}</Badge>
                          </div>
                          <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">{entry.body}</p>
                        </div>
                      </div>
                    );
                  })}

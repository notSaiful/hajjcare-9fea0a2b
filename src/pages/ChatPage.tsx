...
const ChatPage = () => {
  ...
  const [zoyaLanguageCode, setZoyaLanguageCode] = useState(() => getZoyaLanguage(language).code);
  const { messages, isLoading, sendMessage, clearChat, currentModule } = useAIOrchestrator(zoyaLanguageCode);
          ...
          >
            ...
            <TabsContent value="chat" className="mt-0">
              <div className="mb-4 flex items-center gap-2 rounded-lg border border-border bg-card p-2">
                <label htmlFor="zoya-chat-language" className="shrink-0 text-xs font-medium">Zoya language</label>
                <Select value={zoyaLanguageCode} onValueChange={setZoyaLanguageCode} disabled={isLoading}>
                  <SelectTrigger id="zoya-chat-language" aria-label="Choose Zoya response language" className="h-9 flex-1">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="max-h-72">
                    {ZOYA_LANGUAGES.map((zoyaLanguage) => (
                      <SelectItem key={zoyaLanguage.code} value={zoyaLanguage.code} dir={zoyaLanguage.dir}>
                        {zoyaLanguage.name} · {zoyaLanguage.nativeName}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {hasMessages ? (
                <div className="space-y-4">
                  ...
                  {/* Input */}
                  <ChatInput onSend={sendMessage} isLoading={isLoading} languageCode={zoyaLanguageCode} />
                </div>
              ) : (

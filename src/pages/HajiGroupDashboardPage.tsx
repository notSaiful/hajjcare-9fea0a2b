...
const HajiGroupDashboardPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    const typedPilgrim = pilgrimData as unknown as PilgrimRecord;
    setPilgrim(typedPilgrim);

    // Fetch group info
    const { data: groupData, error: groupError } = await supabase
      .from('inspector_pilgrim_groups')
      .select('id, group_name, whatsapp_group_link, inspector_user_id')
      .eq('id', typedPilgrim.group_id)
      .single();

    if (!groupError && groupData) setGroup(groupData as unknown as GroupInfo);

    // Fetch announcements
    const { data: announcementsData, error: announcementsError } = await supabase
      .from('group_announcements')
      .select('id, title, message, priority, created_at')
      .eq('group_id', typedPilgrim.group_id)
      .order('created_at', { ascending: false })
      .limit(20);

    if (!announcementsError) setAnnouncements((announcementsData || []) as unknown as Announcement[]);
    setLoading(false);
  }, [user]);
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {group?.whatsapp_group_link && (
            <Button
              ...
              onClick={() => window.open(group.whatsapp_group_link!, '_blank', 'noopener,noreferrer')}
            >

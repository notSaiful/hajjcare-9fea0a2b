...
export default function AdminPromoPage() {
  ...
  const handleCreate = async () => {
    ...
    if (!/^[A-Z0-9_-]{3,32}$/.test(formCode.trim().toUpperCase())) {
      toast({ title: "Invalid promo code", description: "Use 3–32 letters, numbers, hyphens, or underscores.", variant: "destructive" });
      return;
    }
    const discountValue = Number(formValue);
    if (!Number.isFinite(discountValue) || discountValue < 0 || (formType === "percentage" && discountValue > 100)) {
      toast({ title: "Invalid discount", description: "Enter a valid discount value.", variant: "destructive" });
      return;
    }
    setSubmitting(true);
    try {
        ...
        <div className="px-4 max-w-2xl mx-auto space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <Card className="p-3 text-center">
              ...
              <p className="text-xl font-bold text-foreground">{promoCodes.filter((code) => code.is_active).length}</p>
              <p className="text-xs text-muted-foreground">Active Codes</p>
            </Card>
            <Card className="p-3 text-center">
              <Award className="w-5 h-5 text-amber-500 mx-auto mb-1" />
              <p className="text-xl font-bold text-foreground">{promoCodes.length - promoCodes.filter((code) => code.is_active).length}</p>
              <p className="text-xs text-muted-foreground">Inactive Codes</p>
            </Card>
          </div>

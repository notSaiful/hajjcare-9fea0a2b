...
const labels: Record<string, LocalizedString> = {
  ...
  listen: { en: "Listen", ar: "استمع", ur: "سنیں", hi: "सुनें" },
...
const RulesBriefingPage = () => {
  ...
  const translationItems = useMemo(() => [
    // Keep the page chrome in the first batch so the heading, controls, and
    // fallback notice switch immediately when a pilgrim changes language.
    ...Object.entries(labels).map(([key, value]) => ({ key: `ui.${key}`, text: value.en, nativeText: value[language] })),
    ...RULES_SECTIONS.flatMap((section) => [
  ...
  const audioItems = useMemo<AccessibleRuleAudioItem[]>(() => RULES_SECTIONS.flatMap((section) => [
    {
      id: `${section.id}.intro`,
      anchorId: section.id,
      label: translatedText(`section.${section.id}.title`),
      text: `${translatedText(`section.${section.id}.title`)}. ${translatedText(`section.${section.id}.description`)}`,
    },
  ]), [translatedText]);
  const handleAudioActive = useCallback((sectionId: string | undefined) => {
    setActiveSection(sectionId);
    if (sectionId) document.getElementById(`rules-section-${sectionId}`)?.scrollIntoView({ behavior: "smooth", block: "nearest" });
  }, []);
      ...
      <main className="container max-w-lg mx-auto px-4 py-6 space-y-6 pb-32">
        ...
        <div className="space-y-2">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h1 className="text-2xl font-bold">{translatedText("ui.title")}</h1>
              <p className="text-muted-foreground">{translatedText("ui.subtitle")}</p>
            </div>
          </div>
            ...
            return (
              <Card
                ...
                className={`border-2 transition-all cursor-pointer hover:border-primary/50 ${
                  activeSection === section.id ? "border-primary bg-primary/10 ring-2 ring-primary/30" : isRead ? "border-status-safe/30 bg-status-safe/5" : "border-border"
                }`}
                ...
                <CardContent className="p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`font-semibold ${isRead ? "text-status-safe" : "text-foreground"}`}>
                      {translatedText(`section.${section.id}.title`)}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {translatedText(`section.${section.id}.description`)}
                    </p>
                  </div>
          ...
          <div className="container max-w-lg mx-auto">
            {confirmed ? (
              <div className="flex items-center justify-center gap-2 p-3 bg-status-safe/10 rounded-xl border border-status-safe/30">
                ...
                <span className="font-medium text-status-safe">{translatedText("ui.confirmed")}</span>
              </div>
            ) : allRead ? (
              <Button className="w-full h-12 text-base" onClick={handleConfirm}>
                <Check className="w-5 h-5 mr-2" />
                {translatedText("ui.confirm")}
              </Button>
            ) : (
              <div className="text-center text-muted-foreground py-2">
                {translatedText("ui.readAll")}
              </div>
            )}
          </div>
        </div>

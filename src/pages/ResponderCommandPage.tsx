...
type ResponderProfile = Database["public"]["Tables"]["responder_profiles"]["Row"];
...
type HierarchyAuditLog = Database["public"]["Tables"]["hierarchy_audit_log"]["Row"];
...
export default function ResponderCommandPage() {
  ...
  const [auditLogs, setAuditLogs] = useState<HierarchyAuditLog[]>([]);
  const [allResponders, setAllResponders] = useState<ResponderProfile[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    const [profileRes, modulesRes, recordsRes, deploymentsRes] = await Promise.all([
      supabase.from("responder_profiles").select("*").eq("user_id", user.id).maybeSingle(),
      supabase.from("training_modules").select("*").eq("is_active", true).order("sort_order"),
      supabase.from("training_records").select("*").eq("user_id", user.id),
      supabase.from("deployment_logs").select("*").eq("user_id", user.id).order("created_at", { ascending: false }).limit(20),
    ]);
    ...
    if (hasAnyCoordinatorRole) {
      const [auditRes, respondersRes] = await Promise.all([
        supabase.from("hierarchy_audit_log").select("*").order("created_at", { ascending: false }).limit(50),
        supabase.from("responder_profiles").select("*").order("rank"),
      ]);
  ...
  const createProfile = async () => {
    ...
    const { error } = await supabase.from("responder_profiles").insert({
      user_id: user.id,
      rank: "trainee",
      specialty: [],
      languages: [],

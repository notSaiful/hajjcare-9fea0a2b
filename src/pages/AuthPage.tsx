    ...
    >
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" size="sm" className="absolute right-4 top-4 z-20 min-h-11 bg-card/90" aria-label="Choose language">
            <Globe className="mr-2 h-4 w-4" />
            {LANGUAGES.find((item) => item.code === language)?.nativeName}
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align={isRTL ? "start" : "end"} className="z-30 max-h-80 overflow-y-auto">
          {LANGUAGES.map((item) => (
            <DropdownMenuItem key={item.code} onClick={() => setLanguage(item.code)} className="min-h-11 justify-between">
              {item.nativeName}
      ...
      >
        ...
        <CardContent className="px-5 sm:px-6 py-6">
          <EmailAuthForm onSuccess={() => navigate(returnTo, { replace: true })} />
